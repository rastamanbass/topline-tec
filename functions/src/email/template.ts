...
// ── Header with logo ───────────────────────────────────────────────────────────
function headerBlock(tagline: string, badge?: string): string {
  return `
...
export function invoiceEmailHtml(inv: InvoiceData): string {
  // Items rows
...
// ── TEMPLATE 2: Shipment status update ────────────────────────────────────────
...
const SHIPMENT_ICONS: Record<string, string> = {
  "preparando":     "📦",
  "en_bodega_usa":  "🏭",
  "en_transito":    "✈️",
  "en_aduana":      "🛃",
...
export function shipmentEmailHtml(data: ShipmentEmailData, statusKey?: string): string {
  const icon   = statusKey ? (SHIPMENT_ICONS[statusKey] ?? "📬") : "📬";
  const colors = statusKey
    ? (STATUS_COLORS[statusKey] ?? { bg: BLUE_LIGHT, border: BLUE, text: BLUE })
