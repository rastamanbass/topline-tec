...
// ── Core builder ───────────────────────────────────────────────────────────────
function buildDoc(invoice: InvoiceData): jsPDF {
  ...
  if (invoice.clientPhone) { doc.text(invoice.clientPhone, colLeft, y); y += 4.5; }
  if (invoice.clientEmail) { doc.text(invoice.clientEmail, colLeft, y); y += 4.5; }
 ...
 */
export async function buildPdfBase64(invoice: InvoiceData): Promise<string> {
