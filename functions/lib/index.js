...
function parseBrandModelFromHtml(html) {
    const brands = ["Apple", "Samsung", "Xiaomi", "Motorola", "Google", "Huawei", "Oppo", "Vivo", "OnePlus", "Sony", "LG"];
    for (const brand of brands) {
        if (html.toLowerCase().includes(brand.toLowerCase())) {
            ...
            if (match === null || match === void 0 ? void 0 : match[1]) {
                ...
                for (const g of ["price", "specs", "review", "buy", "imei", "tac", "check", "support", "compare"]) {
...
exports.lookupTac = (0, https_1.onCall)({ cors: true }, async (request) => {
    ...
    const phonesSnap = await db.collection("phones")
        .where("imei", ">=", tac)
        .where("imei", "<", tac + "\uf8ff")
        .limit(1)
        .get();
    if (!phonesSnap.empty) {
        ...
        if (d.marca && d.modelo) {
            await db.doc(`device_definitions/${tac}`).set({ brand: d.marca, model: d.modelo, updatedAt: Date.now() });
    ...
    try {
        ...
        if (result) {
            await db.doc(`device_definitions/${tac}`).set({ brand: result.brand, model: result.model, updatedAt: Date.now() });
            return result;
        }
    }
    catch (e) {
